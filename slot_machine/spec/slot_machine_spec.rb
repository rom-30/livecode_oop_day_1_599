...
def test_scenario(reels, expected_results)
  it "returns #{expected_results} for #{reels[0]} | #{reels[1]} | #{reels[2]}" do
    slot_machine = SlotMachine.new(reels)

    score = slot_machine.score
    expect(score).to eq(expected_results)
  end
end

describe SlotMachine do
  describe '#score' do
    it 'returns 0 for three different elements' do
